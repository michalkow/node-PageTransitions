...
var PageTransitions = function(options) {
	...
	self.$pages = self.$main.children( self.pages );
	self.animcursor = 1;
	...
	self.animEndEventName = self.animEndEventNames[ Modernizr.prefixed( 'animation' ) ];
	// self.support css animations
...
PageTransitions.prototype.nextPage = function ( animation, cb ) {
	var self = this;

	if( self.isAnimating || animation > 67 ) {
		return false;
	}
	...
	var $nextPage = self.$pages.eq( self.current ).addClass( 'pt-page-next' ),
		outClass = '', inClass = '';
	...
	$currPage.addClass( outClass ).on( self.animEndEventName, function() {
		...
		if( self.endNextPage ) {
			self.onEndAnimation( $currPage, $nextPage, cb );
		}
	} );
